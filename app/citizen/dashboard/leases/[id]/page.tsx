...
export default function LeaseRequestDetailPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "PENDING_SUPERVISOR_APPROVAL":
        return (
          <Badge className="bg-orange-100 text-orange-800 hover:bg-orange-100 border border-orange-200 text-xs font-semibold">
            <Clock className="w-3 h-3 mr-1" />
      ...
      case "SUPERVISOR_APPROVED":
        return (
          <Badge className="bg-green-100 text-green-800 hover:bg-green-100 border border-green-200 text-xs font-semibold">
            <CheckCircle2 className="w-3 h-3 mr-1" />
            Active
          </Badge>
        );
  ...
  if (error) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-xl p-6 text-center">
        ...
        <Button
          variant="outline"
          onClick={() => router.push("/citizen/dashboard/leases")}
          className="mt-4"
        >
          Go Back
        </Button>
    ...
    return (
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-6 text-center">
        ...
        <Link
          ...
          className="inline-flex items-center gap-1.5 font-bold text-slate-600 hover:text-[#00450d] transition-colors text-xs mt-4"
        >
          <ChevronLeft className="w-4 h-4" />
          <span>BACK TO MY LEASES</span>
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-150">
      ...
      {/* Header Info */}
      <div>
        <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Lease Request Details</h1>
        <p className="text-sm text-slate-500">Request Code: <span className="font-mono font-bold text-slate-700">{leaseRequest.requestCode}</span></p>
      </div>
        ...
        <CardContent className="p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              {getStatusBadge(leaseRequest.status)}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="text-slate-600">
                {leaseRequest.propertySubCity + ", " + leaseRequest.propertyWoreda || "Address not specified"}
              </span>
            </div>
            <div className="flex items-center gap-2 text-sm">
      ...
      {(leaseRequest.status === "LANDLORD_APPROVED" || leaseRequest.status === "UNDER_VERIFICATION" || leaseRequest.status === "PENDING_SUPERVISOR_APPROVAL") && (
