...
import { PropertyResponse, PropertyUnitResponse, getPropertyByCode, getSession, submitLeaseRequest } from "@/lib/api";

function SearchDetailPageContent() {
  ...
  const handleApplyLease = (prop: PropertyResponse) => {
    if (selectedUnit && selectedUnit.rentAmount) {
      ...
      setProposedRent(prop.monthlyRent || 0);
    }
    setIsApplyModalOpen(true);
  };

  const handleSelectUnit = (unit: PropertyUnitResponse) => {
    ...
    return (
      <div className="w-full bg-[#f8fafc] text-slate-900 font-sans antialiased min-h-screen flex items-center justify-center p-4">
        <Card className="max-w-md w-full p-8 text-center">
          ...
          <Link
            href="/citizen/dashboard/search"
            className="inline-flex items-center gap-1.5 font-bold text-slate-700 hover:text-[#00450d] transition-colors text-xs border border-slate-200 px-4 py-2 rounded-lg mx-auto"
          >
        ...
        <div className="flex items-center justify-between gap-4 pb-3 border-b border-slate-200">
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="font-mono font-bold text-slate-800">
              {property?.propertyCode || propertyCodeParam || "PROPERTY DETAILS"}
            </span>
          </div>
                  ...
                  <table className="w-full text-xs text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100 text-slate-800">
                      {property.units.map((unit) => {
                        ...
                        const isRented = (unit.status || "").toUpperCase() === "RENTED";
                        const isSelected = selectedUnit?.id === unit.id;

                        return (
                          <tr
                            key={unit.id}
                            onClick={() => isAvailable && handleSelectUnit(unit)}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-emerald-50/70 border border-emerald-200/80 rounded-2xl p-4 text-xs text-emerald-950 space-y-2">
              <Button
                ...
                disabled={Boolean(property?.units && property.units.length > 0 && !selectedUnit)}
                className={`w-full flex-1 text-xs font-medium h-9 rounded-lg flex items-center justify-center gap-1.5 shadow-2xs ${
                  property?.units && property.units.length > 0 && !selectedUnit
              ...
              {property?.units && property.units.length > 0 && !selectedUnit && (
                <p className="text-center text-[10px] text-amber-700 font-medium">
                  {property.units.every((u) => (u.status || "").toUpperCase() !== "AVAILABLE")
                    ? "All units in this building are currently rented"
                    : "Please select an available unit above to apply for lease"}
                </p>
              )}
              {selectedUnit && (
                <Button
...
export default function SearchDetailPage() {
  return (
    <Suspense fallback={<div className="p-8 text-center text-sm text-slate-500">Loading search details...</div>}>
