...
export const SideNavBar: React.FC<SideNavBarProps> = ({
  ...
  const citizenNavItems: Array<{
    ...
    { id: "landing", label: "Home Portal", icon: Home, category: "Overview" },
    { id: "dashboard", label: "Dashboard", icon: LayoutDashboard, category: "Overview" },
    ...(userRole === "tenant"
      ? [
          { id: "search" as NavPage, label: "Search House", icon: Search, category: "Rentals" },
          { id: "agreements" as NavPage, label: "My Lease Requests", icon: FileText, category: "Rentals", badge: pendingAgreementsCount },
        ]
      : [
          { id: "properties" as NavPage, label: "My Properties", icon: Building2, category: "Management" },
          { id: "register-property" as NavPage, label: "+ Register Property", icon: PlusCircle, category: "Management" },
          { id: "agreements" as NavPage, label: "Rental Agreements", icon: FileText, category: "Management", badge: pendingAgreementsCount },
        ]),
    { id: "payments", label: "Payments", icon: CreditCard, category: "Finance" },
    { id: "bills", label: "Bills & Invoices", icon: Receipt, category: "Finance" },
    { id: "profile", label: "My Profile", icon: User, category: "Account" },
  ];
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-white border-r border-slate-200/90 text-slate-900 select-none">
      ...
      {/* Support & Archive Bottom Links (Matching Images 1, 2, 3, 4) */}
      <div className="py-2 border-t border-slate-100 space-y-0.5">
        <button
          onClick={() => alert("Woreda Municipal Support Desk: Toll-free 8812 • support@woreda.gov.et")}
        ...
        <button
          onClick={() => alert("Municipal Archive: Accessing Proclamation No. 1320 registry...")}
          className="w-full flex items-center gap-3 px-4 py-2 text-xs font-medium text-slate-700 hover:bg-slate-50 hover:text-slate-900 transition-colors text-left"
        >
          <Archive className="w-4 h-4 text-slate-500" />
          {!collapsed && (
            <span className="text-xs">
              {isSupervisor ? "Official Registry (Archive)" : "Archive"}
            </span>
          )}
        </button>
      </div>
            ...
            <div className="min-w-0 pr-1">
              <p className="text-xs font-bold text-slate-900 truncate">
                {session?.user
                  ...
                  : isOfficer
                  ? isSupervisor ? "Supervisor User" : "Officer Abebe Bikila"
                  : userRole === "landlord"
                  ? "Dagmawit Mesfin"
                  : "Sara Tekle"}
              </p>
              <p className="text-[10px] text-slate-500 truncate">
                {isOfficer
                  ? isSupervisor ? "Woreda Supervisor Desk" : "Woreda 03 Officer Desk"
                  : userRole === "landlord"
                  ? "Registered Landlord"
                  : "Verified Citizen Tenant"}
              </p>
            </div>
            <button
              onClick={onLogoutClick}
              className="flex items-center gap-1 text-xs font-bold text-rose-700 hover:bg-rose-50 px-2 py-1 rounded-md transition-colors shrink-0 uppercase tracking-wider text-[10px]"
              title="Logout"
            >
    ...
    <>
      ...
      {isMobileOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          />
          <div className="fixed inset-y-0 left-0 w-[240px] z-50 bg-white shadow-xl animate-in slide-in-from-left duration-200">
            {sidebarContent}
          </div>
        </div>
      )}
